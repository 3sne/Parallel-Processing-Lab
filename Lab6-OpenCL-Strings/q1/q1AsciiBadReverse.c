...
int main() {
	...
	scanf("%s", buf);
	LS = strlen(buf);
	char *A = (char *)malloc(sizeof(int)*LS);
	strcpy(A, buf);
	char *B = (char *)malloc(sizeof(int)*LS);
	...
	cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, LS*sizeof(char),NULL, &ret);
	cl_mem b_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, LS*sizeof(char),NULL, &ret);
